import { ISearchStrategy, ISearchStrategyResult } from "../../ISearchStrategy.js";
...
export class DefaultSearchStrategy implements ISearchStrategy {
    ...
    async search(base_dir: string): Promise<ISearchStrategyResult[]> {
        ...
        return fileResults.filter(([_helmPath, _packageJsonPath, hasPackageJSON]) => hasPackageJSON).map(([helmPath, packageJsonPath]) => ({
