...
const chartRenderer = new SecretRenderer(new class implements IConfigMapNameProvider {
...
describe('A secret renderer test', () => {
    ...
    it('Should produce a file (global)', async () => {
        const result = await chartRenderer.render(undefined, new Map([[undefined, new Map([['hello', { type: 'global', default: 'ehllo', secret: true }], ['unused', { type: 'global', default: 'should not be present!', secret: true }]])]]), new Map([['world', 'hello']]));

        writers.push(result);
    });

    it ('Should produce a file (scoped/workspace)', async () => {
        const result = await chartRenderer.render(workspace, new Map([[workspace, new Map([['hello2', { type: 'scoped', default: 'ehllo', secret: true }], ['unused2', { type: 'scoped', default: 'should not be present!', secret: true }]])]]), new Map([['world2', 'hello2']]));
