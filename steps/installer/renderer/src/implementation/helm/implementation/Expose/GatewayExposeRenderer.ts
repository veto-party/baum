...
export class GatewayExposeRenderer implements IExposeRenderer {
  ...
  render(workspace: IWorkspace, config: Map<string | number, ExposeStructure> | undefined): IExposeRenderResult {
    const gatewayConfigs = (name: string) => Array.from(config?.entries?.() ?? [])
          .map(([port, exposed]) => {
            ...
            return  {
                ...
                spec: {
                    ...
                    rules: [{
                        ...
                        filters: (exposed.doNotStripPrefix || !exposed.matcher?.path) ? undefined : [{
                            type: 'URLRewrite',
                            urlRewrite: {
                                path: {
    ...
    return {
      ...
      write: async (root, resolver) => {
        ...
        const fileContent = gatewayConfigs(path).filter(<T>(el: T|undefined): el is T => el !== undefined)
          .flat()
