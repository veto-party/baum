...
import { AToken } from "../yaml/AToken.js";
...
export class TraefikExposeRenderer implements IExposeRenderer {
    ...
    public constructor(
        private entryPoint: string | AToken
    ) {}
    ...
    render(workspace: IWorkspace, config: Map<string | number, ExposeStructure> | undefined): IExposeRenderResult {
        ...
        const cors =  (name: string) => Object.fromEntries(
          ...
          const pathFilters = (name: string) => Object.fromEntries(
            (Array.from(config?.entries?.() ?? []))
              .map(([port, exposed]) => {
                if (exposed.type === 'internal') {
                  return;
        ...
        return {
            ...
            write: async (root, resolver) => {
                ...
                await FileSystem.writeFile(Path.join(fileContent, 'ingress.yaml'), fileContent);
            }
        }
    }
}