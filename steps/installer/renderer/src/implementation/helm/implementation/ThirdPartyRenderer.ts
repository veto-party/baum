    ...
    return {
      getConfigMap: async (resolver: INameProvider): Promise<Map<string, any>> => {
        ...
        return HelmRenderer.mergeElements(
          new Map(dependencies.entries().flatMap(([key, values]) => Object.entries(values.properties ?? {}).map(([baseKey, value]) => [[name, key, baseKey].filter(Boolean).join('.'), value] as const))),
