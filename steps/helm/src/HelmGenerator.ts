...
import { SchemaType, schema } from "./types/types.js";
import { flattenKeys } from "./utility/formatConverter.js";
...
class HelmGenerator implements IStep {
    ...
    private async collectAllWorkspaces(pm: IExecutablePackageManager, root: string): Promise<Map<IWorkspace, IWorkspace[]>> {
        ...
        do {
            ...
            const filesToCheck = await Promise.all(directoriesToCheck.map((directory) => FileSystem.readFile(Path.join(directory, 'package.json')).then(async (content) => [await FileSystem.realpath(directory), content] as const).catch(() => undefined)));
            ...
            await Promise.all(resultingContents.map(([directory, content]) => {
                const newWorkspace = new GenericWorkspace(directory, JSON.parse(content.toString()), pm.modifyToRealVersionValue.bind(pm));

                let internalWorkspace = [...internalWorkspaces, ...Object.values(pathToWorkspace)].find((internalWorkspace) => internalWorkspace.getDirectory() === directory || internalWorkspace.getVersion() === newWorkspace.getVersion() || semver.satisfies(newWorkspace.getVersion(), internalWorkspace.getVersion())) ?? newWorkspace;
    ...
    private async getContext(workspace: IWorkspace, map: HelmFileResult, layer: number = 1) {
