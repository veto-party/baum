...
import type {
  ExtendedSchemaType,
  HelmGeneratorProvider,
} from "./HelmGeneratorProvider.js";
...
export class HelmGenerator implements IStep {
  constructor(
    ...
    private dockerFileForJobGenerator: (
      schema: Exclude<SchemaType["job"], undefined>[string],
  ...
  @CachedFN(true)
  async generateGlobalScope(
    contexts: Map<IWorkspace, ExtendedSchemaType>,
    context: ExtendedSchemaType,
    ...
    const allBindings = Array.from(contexts.values()).flatMap((definition) =>
      Object.entries(
        resolveBindings(definition.binding ?? {}, definition, context),
      ).filter(([, value]) => value.is_global),
    );

    const finalizedSCopedContext = Array.from(contexts.entries()).reduce(
      (prev, [workspace, schema]) =>
        this.helmFileGeneratorProvider.groupScopes([schema, prev], workspace),
      {
        variable: {},
        is_service: false,
    ...
    allBindings.push(
      ...Object.entries(
        resolveBindings(context.binding ?? {}, finalizedSCopedContext, context),
      ),
    );

    [
      ...Array.from(contexts.values()).map(
        (context) => [context, false] as const,
      ),
      [context, true] as const,
    ].forEach(([current, is_global]) => {
      Object.values(current.job ?? {}).forEach((job) => {
        allBindings.push(
          ...Object.entries(
            resolveBindings(
              job.binding ?? {},
              !is_global ? (job.variable ? [current, job] : current) : current,
              is_global ? (job.variable ? [context, job] : context) : context,
              is_global,
            ),
          ),
        );
    ...
    allBindings.forEach(([key, resolved]) => {
      if ((resolved.is_global || resolved.external) && !resolved.static) {
        set(
          valuesYAML,
          (resolved.is_global && !resolved.external ? "global." : "") +
            resolved.referenced,
          buildVariable(resolved, "global"),
        );
      }
    });
    ...
    const secretsYAML = {
      ...
      stringData: Object.fromEntries(
        allBindings
          .filter(
            ([, value]) => !value.static && value.secret && value.is_global,
          )
          .map(([key, value]) => {
            return [
              key,
              new RawToken(
                `{{.Values.${HelmGenerator.buildVariablePath(value.referenced, ".")} | quote }}`,
              ),
            ];
          }),
      ),
    ...
    if (Object.keys(secretsYAML.stringData).length > 0) {
      await this.writeObjectToFile(
        rootDirectory,
        ["helm", "main", "templates", "secret.yaml"],
        [secretsYAML],
      );
    }
        ...
        allBindings
          ...
          .map(([key, value]) => {
            return [
              key,
              new RawToken(
                `{{.Values.${value.external ? "" : "global."}${HelmGenerator.buildVariablePath(value.referenced, ".")} | quote }}`,
              ),
            ];
          }),
      ),
    ...
    if (Object.keys(configMapYAML.data).length > 0) {
      await this.writeObjectToFile(
        rootDirectory,
        ["helm", "main", "templates", "configmap.yaml"],
        [configMapYAML],
      );
    }
      ...
      metadata: {
        ...
        annotations: {
          "helm.sh/hook": entry.definition?.on
            ...
            : new RawToken("post-install, post-upgrade"),
          "helm.sh/hook-delete-policy": new RawToken(
        ...
        template: {
          spec: {
            ...
            containers: [
              {
                name: `${key}-container`,
                image: this.dockerFileForJobGenerator(
                  { ...entry, workspace: undefined } as Exclude<
                    SchemaType["job"],
                    undefined
                  >[string],
                  entry.workspace,
                ...
                env: Object.entries(
                  resolveBindings(
                    entry?.binding ?? {},
                    [],
                    entry.variable !== undefined ? [entry, context] : context,
                    true,
                  ),
                ).map(([key, resolved]) => {
                  if (resolved.static) {
  ...
  async execute(
    workspace: IWorkspace,
    packageManager: IExecutablePackageManager,
    rootDirectory: string,
  ): Promise<void> {
    const scopedContext =
      this.helmFileGeneratorProvider.contexts.get(workspace);
    const globalContext = this.helmFileGeneratorProvider.globalContext;

    if (!scopedContext?.is_service) {
      console.info(
        `Workspace: ${JSON.stringify(workspace.getName())} is not a service. No helm files will be generated for it.`,
      );
      return;
    }

    await this.generateGlobalScope(
      this.helmFileGeneratorProvider.contexts,
      this.helmFileGeneratorProvider.globalContext,
      rootDirectory,
    );
    ...
    await this.writeObjectToFile(
      ...
      [
        "helm",
        "subcharts",
        workspace.getName().replaceAll("/", "__"),
        "Chart.yaml",
      ],
      [ChartYAML],
    );
    ...
    const allBindings = Object.entries(
      resolveBindings(
        scopedContext.binding ?? {},
        scopedContext,
        globalContext,
      ),
    );
    ...
    Object.values(scopedContext.job ?? {}).forEach((job) => {
      allBindings.push(
        ...Object.entries(
          resolveBindings(job?.binding ?? {}, scopedContext, globalContext),
        ),
      );
    });
    ...
    await this.writeObjectToFile(
      ...
      [
        serviceYAMLInternal.spec.ports.length > 0
          ? serviceYAMLInternal
          : undefined,
        serviceYAMLExternal.spec.ports.length > 0
          ? serviceYAMLExternal
          : undefined,
      ].filter(Boolean),
    );
      ...
      spec: {
        ...
        routes: Object.entries(scopedContext?.expose ?? {})
          ...
          .map(([port, exposed]) => ({
            kind: "Rule",
            match: new RawToken(
              `Host(\`${exposed.domainPrefix}{{.Values.global.host.domain}}\`) && PathPrefix(\`${exposed.path}\`)`,
            ),
            services: [
              {
                name: `${name.replaceAll("_", "-")}-ext`,
    ...
    await this.writeObjectToFile(
      ...
      [
        ingressYAMLStripPrefixes,
        Object.keys(ingressYAMLRoutes.spec.routes).length > 0
          ? ingressYAMLRoutes
          : undefined,
      ]
        .filter(Boolean)
    ...
    deploymentYAML.spec.template.spec.containers.forEach(
      (c) => !c.resources && delete c.resources,
    );
        ...
        allBindings
          ...
          .map(([key, value]) => {
            return [
              key,
              new RawToken(
                `{{.Values.${value.is_global ? "global." : ""}${HelmGenerator.buildVariablePath(value.referenced, ".")} | quote }}`,
              ),
            ];
          }),
      ),
    ...
    const jobYAML = Object.entries(scopedContext?.job ?? {}).map(
      ([key, entry]) => ({
        apiVersion: "batch/v1",
        kind: "Job",
        metadata: {
              ...
              containers: [
                {
                  ...
                  env: Object.entries(
                    resolveBindings(
                      entry?.binding ?? {},
                      entry.variable ? [scopedContext, entry] : scopedContext,
                      globalContext,
                    ),
                  ).map(([key, resolved]) => {
                    if (resolved.static) {
    ...
    const configuration = Object.entries(
      scopedContext.scaling?.configuration ?? {},
    );
