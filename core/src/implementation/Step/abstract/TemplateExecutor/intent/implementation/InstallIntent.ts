...
class InstallIntent extends AIntent implements IInstallIntent {
    ...
    getPrepareStep = (): IStep => {
        ...
        const npmRCCopy = new CopyStep((_, __, rootDirectory) => FileSystem.existsSync(Path.join(rootDirectory, '.npmrc')) ? [Path.join(rootDirectory, '.npmrc')] : [], (workspace) => workspace.getDirectory());

        if (this.keep) {
            return new ParallelStep([
                npmRCCopy,
                new CopyStep((_, pm, root) => [Path.join(root, pm.getLockFileName())], (workspace, filename) => Path.join(workspace.getDirectory(), Path.basename(filename)))
            ])
        }
