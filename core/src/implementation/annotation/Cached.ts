...
export const CachedFN = <T extends (...args: any[]) => any>(async: ReturnType<T> extends Promise<any> ? true : false) => {
    return (_target: any, __propertyKey: string, context: TypedPropertyDescriptor<T>) => {
        ...
        if (async) {
            ...
            const resolveOrReject = <Index extends 0 | 1>(index: Index) => (value: Parameters<typeof promises[number][typeof index]>[0]) => promises.forEach((promiseTuple) => promiseTuple[index](value));
